...
public static class ConsoleInput
{
    ...
    public static string ToStringOrEmptySerializer<T>(T origin) { return origin?.ToString() ?? ""; }
    ...
    (
        ...
        Func<int, T, string>? labeler = null
    )
    {
        serializer ??= ToStringOrEmptySerializer;
        ...
        for (;;)
        {
            ...
            if (ans is not null && labels.TryGetValue(ans, out var value)) return selectFrom[value];
        }
    }
    ...
    private static IEnumerable<int> SpaceHyphenParserInternal
        (string? userInput, IReadOnlyDictionary<string, int> labelMapping)
    {
    ...
    /// <param name="default">
    ///     NULL if only y/n is allowed; Otherwise, if user input is null or empty, such default value will
    ///     be returned.
    /// </param>

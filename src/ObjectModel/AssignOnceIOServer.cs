...
namespace PlasticMetal.MobileSuit.ObjectModel
{
    ...
    public class AssignOnceIOServer : AssignOnce<IIOServer>, IAssignOnceIOServer
    {
        ...
        /// <inheritdoc />
        public void ResetError() => (Element ?? IIOServer.GeneralIO).ResetError();

        /// <inheritdoc />
        public void ResetOutput() => (Element ?? IIOServer.GeneralIO).ResetOutput();
        ...
        public Task WriteAsync(string content, ConsoleColor frontColor, ConsoleColor backColor)
            => (Element ?? IIOServer.GeneralIO).WriteAsync(content, frontColor, backColor);

        /// <inheritdoc />
        public Task WriteAsync(string content, ConsoleColor? customColor)
            => (Element ?? IIOServer.GeneralIO).WriteAsync(content, customColor);

        /// <inheritdoc />
        public Task WriteAsync(string content, OutputType type = OutputType.Default, ConsoleColor? customColor = null)
            => (Element ?? IIOServer.GeneralIO).WriteAsync(content, type, customColor);
        ...
        public Task WriteLineAsync()
            => (Element ?? IIOServer.GeneralIO).WriteLineAsync();

        /// <inheritdoc />
        public Task WriteLineAsync(string content, ConsoleColor customColor)
            => (Element ?? IIOServer.GeneralIO).WriteLineAsync(content, customColor);

        /// <inheritdoc />
        public Task WriteLineAsync(string content, OutputType type = OutputType.Default,
            ConsoleColor? customColor = null)
            => (Element ?? IIOServer.GeneralIO).WriteLineAsync(content, type, customColor);

        /// <inheritdoc />
        public Task WriteLineAsync(IEnumerable<(string, ConsoleColor?)> contentArray,
            OutputType type = OutputType.Default)
            => (Element ?? IIOServer.GeneralIO).WriteLineAsync(contentArray, type);
        /// <inheritdoc />
        public Task WriteLineAsync(IAsyncEnumerable<(string, ConsoleColor?)> contentArray, OutputType type = OutputType.Default)
            => (Element ?? IIOServer.GeneralIO).WriteLineAsync(contentArray, type);
        
        /// <inheritdoc />
        public Task WriteLineAsync(IEnumerable<(string, ConsoleColor?, ConsoleColor?)> contentArray, OutputType type = OutputType.Default)
            => (Element ?? IIOServer.GeneralIO).WriteLineAsync(contentArray, type);
        /// <inheritdoc />
        public Task WriteLineAsync(IAsyncEnumerable<(string, ConsoleColor?, ConsoleColor?)> contentArray, OutputType type = OutputType.Default)
            => (Element ?? IIOServer.GeneralIO).WriteLineAsync(contentArray, type);
        ...
        public string? ReadLine(string? prompt, bool newLine, ConsoleColor? customPromptColor = null)
            => (Element ?? IIOServer.GeneralIO).ReadLine(prompt, newLine, customPromptColor);

        /// <inheritdoc />
        public string? ReadLine(string? prompt, ConsoleColor? customPromptColor)
            => (Element ?? IIOServer.GeneralIO).ReadLine(prompt, customPromptColor);

        /// <inheritdoc />
        public string? ReadLine(string? prompt, string? defaultValue, ConsoleColor? customPromptColor)
            => (Element ?? IIOServer.GeneralIO).ReadLine(prompt, defaultValue, customPromptColor);

        /// <inheritdoc />
        public string? ReadLine(string? prompt = null, string? defaultValue = null, bool newLine = false,
            ConsoleColor? customPromptColor = null)
            => (Element ?? IIOServer.GeneralIO).ReadLine(prompt, defaultValue, newLine, customPromptColor);

        /// <inheritdoc />
        public Task<string?> ReadLineAsync(string? prompt, bool newLine, ConsoleColor? customPromptColor = null)
            => (Element ?? IIOServer.GeneralIO).ReadLineAsync(prompt, newLine, customPromptColor);

        /// <inheritdoc />
        public Task<string?> ReadLineAsync(string? prompt, ConsoleColor? customPromptColor)
            => (Element ?? IIOServer.GeneralIO).ReadLineAsync(prompt, customPromptColor);
        /// <inheritdoc />
        public Task<string?> ReadLineAsync(string? prompt, string? defaultValue, ConsoleColor? customPromptColor)
            => (Element ?? IIOServer.GeneralIO).ReadLineAsync(prompt,defaultValue, customPromptColor);
        /// <inheritdoc />
        public Task<string?> ReadLineAsync(string? prompt = null, string? defaultValue = null, bool newLine = false,
            ConsoleColor? customPromptColor = null)
            => (Element ?? IIOServer.GeneralIO).ReadLineAsync(prompt, defaultValue,newLine, customPromptColor);

        /// <inheritdoc />
        public int Peek()
            => (Element ?? IIOServer.GeneralIO).Peek();

        /// <inheritdoc />
        public int Read()
            => (Element ?? IIOServer.GeneralIO).Read();

        /// <inheritdoc />
        public string ReadToEnd()
            => (Element ?? IIOServer.GeneralIO).ReadToEnd();

        /// <inheritdoc />
        public Task<string> ReadToEndAsync()
            => (Element ?? IIOServer.GeneralIO).ReadToEndAsync();
    }
}
